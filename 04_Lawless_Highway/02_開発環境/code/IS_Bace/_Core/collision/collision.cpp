...
{
	m_vtxMin = IS_Utility::VEC3_ZERO;
	m_vtxMax = IS_Utility::VEC3_ZERO;
}
	...
	const D3DXVECTOR3 vecNor, D3DXVECTOR3 pos0, D3DXVECTOR3 pos1, D3DXVECTOR3 pos2, D3DXVECTOR3 pos3)
{
	...
	//頂点の回転
	pos0 = IS_Utility::RotateVtx(pos0, rot, posCenter);	//pos0
	pos1 = IS_Utility::RotateVtx(pos1, rot, posCenter);	//pos1
	pos2 = IS_Utility::RotateVtx(pos2, rot, posCenter);	//pos2
	pos3 = IS_Utility::RotateVtx(pos3, rot, posCenter);	//pos3
...
CBoxCollider::CBoxCollider()
{
	...
	m_iCollisionReader = nullptr;
	m_type = TYPE::TYPE_COLLISION;
	m_pCur = this;				//俺が最後尾
}
...
void CBoxCollider::CollisionCheck(void)
{
	...
	if (this->m_type != TYPE::TYPE_NONE)
	{
		...
		{
			if (pCollider != this && pCollider->m_type != TYPE::TYPE_NONE)
			{
				ColFloat otherCol;
				...
				if (CollisionAxis(playerCol, posOld.x, otherCol, pCollider->m_type))	//当たり判定
				{
					...
					for (unsigned int cnt = 0; cnt < collResult.collList.size(); cnt++)
					{
			...
			if (pCollider != this && pCollider->m_type != TYPE::TYPE_NONE)
			{
				...
				{//着地した
				 //重複チェック
					...
					if (pCollider->m_type == TYPE::TYPE_COLLISION)
					{
...
bool CBoxCollider::CollisionAxis(ColFloat source, const float fPosMainOld, ColFloat dest, const TYPE otherType)
{
	...
	if (source.pPosSubA - source.fSizeSubA < dest.pPosSubA + dest.fSizeSubA &&
		...
		source.pPosSubB + source.fSizeSubB > dest.pPosSubB - dest.fSizeSubB)
	{
		if (this->m_type == TYPE::TYPE_COLLISION && otherType == TYPE::TYPE_COLLISION)
		{//両方ぶつかる
